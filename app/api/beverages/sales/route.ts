...
export async function POST(request: Request): Promise<NextResponse> {
  ...
  try {
    ...
    const sale = await prisma.$transaction(async (tx) => {
       ...
       *
       * Matches the PATCH route beside it and the unified `POST /api/sales`,
       * both of which already set this. Widening a timeout cannot change what
       * is written: the same rows commit, or none do.
       */
      timeout: 15_000,
      maxWait: 5_000,
    });
